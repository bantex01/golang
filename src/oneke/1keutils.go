...
func CreateTest(stack string, testType string, testURL string, testID string) {
	...
	case "http-server":
		...
		body := onekeHTTPTestCreate{
			...
			TestName:            testName,
			ContentRegex:        "listen to your data",
			URL:                 testURL,
			AlertsEnabled:       0,
			BgpMeasurements:     0,
...
func GatherTestsForStack(stack string) map[string]map[string]interface{} {
	...
	for url := range allTests {
		...
		//matched, err := regexp.MatchString("(.+)citrix\\.(?:stg|companyworks|companycloud)?\\.(?:companycloud|com|lol)?", url)
...
func GatherAllTests() map[string]map[string]interface{} {
	...
	/*
		testMap looks like:
		onekeTestData["https://wmcglobal-uk.companycloud.com/en-US/account/login?loginType=company"]["testName"] = "stack=wmcglobal-uk id=standard metric=web_check testname=web_check~https://wmcglobal-uk.companycloud.com/en-US/account/login?loginType=company"
		onekeTestData["https://wmcglobal-uk.companycloud.com/en-US/account/login?loginType=company"]["testID"] = "123567"
		onekeTestData["https://wmcglobal-uk.companycloud.com/en-US/account/login?loginType=company"]["testType"] = "http-server"
	*/
